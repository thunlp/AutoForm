...
@output_parser_registry.register("dummy_agree")
class CommonParserCritic(OutputParser):
    # def parse(self, agent, env, output: LLMResult) -> Union[AgentAction, AgentFinish]:
    def parse(self, output: LLMResult) -> Union[AgentAction, AgentFinish]:
        return AgentCriticism(True, output.content)
...
@output_parser_registry.register("logic-puzzle-debate-critic")
class DebateCriticParser(OutputParser):
    def parse(self, output: LLMResult) -> Tuple[List[int], str]:
        text = output.content
        text = re.sub(r"<.+?>", "", text).strip()
        # has_answer = re.search(r"##.+?##", text)
        has_answer = re.search(r"Answer: ?(.+)", text)
        # has_answer = None
        return AgentCriticism(has_answer is not None, text)


@output_parser_registry.register("debate-internal-critic")
class DebateInternalCriticParser(OutputParser):
    def parse(self, output: LLMResult) -> Tuple[List[int], str]:
        text = output.content
        text = re.sub(r"<.+?>", "", text).strip()
        try:
            result = re.findall(r"^Thinking:(.+)Speaking:(.+)", text, re.DOTALL)[0]
            thinking = result[0].strip()
            speaking = result[1].strip()
        except (IndexError, ValueError):
            raise OutputParserError(text)
        # has_answer = re.search(r"##.+?##", text)
        has_answer = re.search(r"Answer: ?(.+)", text)
        # has_answer = None
        return AgentCriticism(has_answer is not None, (speaking, thinking))


@output_parser_registry.register("qa-critic")
class QACriticParser(OutputParser):
    def parse(self, output: LLMResult) -> Tuple[List[int], str]:
        text = output.content
        agree = re.findall(r"<A>.+</A>", text) != []
        return AgentCriticism(agree, text)
