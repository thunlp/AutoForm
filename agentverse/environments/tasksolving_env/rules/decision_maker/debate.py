...
@decision_maker_registry.register("debate")
class DebateDecisionMaker(BaseDecisionMaker):
    """
    ...
    async def astep(
        ...
        **kwargs,
        ...
        for agent_id, agent in enumerate(cycle(agents[1:])):
            ...
            if message.is_agree:
                if (
                    re.findall(r"<A>(.*?)</A>", message.content)[-1] == last_answer
                    or last_answer is None
                ):
                    agree_num += 1
                else:
                    agree_num = 1
                last_answer = re.findall(r"<A>(.*?)</A>", message.content)[-1]
            else:
                last_answer = None
                agree_num = 0
