...
parser.add_argument("--task", type=str, default="tasksolving/logic_grid/autoform-gpt-3.5")
parser.add_argument(
    "--tasks_dir",
    type=str,
...
def check_answer(response, label):
    ans = re.findall(r"[Tt]he answer is.*?(\d)", response)
    if len(ans) == 0:
        return False
    ret_res = normalize_answer(label) == normalize_answer(ans[-1])
    return ret_res
