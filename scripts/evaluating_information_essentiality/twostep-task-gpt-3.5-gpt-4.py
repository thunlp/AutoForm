...
parser.add_argument("--task", type=str, default="tasksolving/evaluating_information_essentiality/twostep-task-gpt-3.5-gpt-4")
parser.add_argument(
    "--tasks_dir",
    type=str,
    default=os.path.join(os.path.dirname(__file__), "..", "..", "agentverse", "tasks"),
)
parser.add_argument("--dataset_path", type=str, default="data/evaluating_information_essentiality/task_all.json")
...
def check_answer(response, label):
    ...
    for l in label:
        if label[l] == 1:
            label = l
            break
    ret_res = label.lower() in ans[-1].lower()
    if not ret_res:
        print(f"Label: {label}")
