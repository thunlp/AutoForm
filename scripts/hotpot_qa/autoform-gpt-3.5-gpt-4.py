...
parser.add_argument("--task", type=str, default="tasksolving/hotpot_qa/gpt-4-cot-model")
parser.add_argument(
    "--tasks_dir",
    type=str,
...
parser.add_argument(
    "--dataset_path",
    type=str,
    default="data/hotpot_qa/test_single.jsonl",
)
...
def _step(index, semaphore, writing_block, example, exist_input, file):
    ...
    with semaphore:
        ...
        llm_type_1 = critic_1.pop("llm_type", "text-davinci-003")
        agentverse.environment.agents[AGENT_TYPES(2)][0].llm = llm_registry.build(
            llm_type_1, **critic_1
        )
        llm_type_2 = critic_2.pop("llm_type", "text-davinci-003")
        agentverse.environment.agents[AGENT_TYPES(2)][1].llm = llm_registry.build(
            llm_type_2, **critic_2
        )
