...
parser.add_argument("--task", type=str, default="tasksolving/coin_flip/autoform-gpt-3.5")
parser.add_argument(
    "--tasks_dir",
    type=str,
    default=os.path.join(os.path.dirname(__file__), "..", "..", "agentverse", "tasks"),
)
parser.add_argument("--dataset_path", type=str, default="data/coin_flip/test.jsonl")
...
def check_answer(logs, label):
    ...
    for log in reversed(logs):
        ...
        ans = re.findall(r"[Tt]he answer is.*?(\w+)", response, re.DOTALL)
        if len(ans) != 0:
            break

    if len(ans) == 0:
        return False
    ret_res = label.lower() == ans[-1].strip().lower()
    return ret_res
