...
parser.add_argument("--task", type=str, default="tasksolving/coin_flip/twostep-task-gpt-4-gpt-3.5")
parser.add_argument(
    "--tasks_dir",
    type=str,
...
def check_answer(response, label):
    ans = re.findall(r'Answer: *\*?\*? *(yes|no)', response, re.IGNORECASE)
    if len(ans) == 0:
        return False
    ret_res = label.lower() == ans[-1].lower()
    if not ret_res:
        print(f"Label: {label}")
