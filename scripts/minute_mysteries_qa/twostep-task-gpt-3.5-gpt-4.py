...
parser.add_argument("--task", type=str, default="tasksolving/minute_mysteries_qa/twostep-task-gpt-3.5-gpt-4.py")
parser.add_argument(
    "--tasks_dir",
    type=str,
...
def check_answer(response, label):
    ans = re.findall(r'Answer: *\*?\*? *(\d+)', response)
    if len(ans) == 0:
        return False
    ret_res = label == int(ans[-1])
    if not ret_res:
        print(f"Label: {label}")
