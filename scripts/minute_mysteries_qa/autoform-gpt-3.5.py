...
parser.add_argument("--task", type=str, default="tasksolving/minute_mysteries_qa/autoform-gpt-3.5")
parser.add_argument(
    "--tasks_dir",
    type=str,
    default=os.path.join(os.path.dirname(__file__), "..", "..", "agentverse", "tasks"),
)
parser.add_argument("--dataset_path", type=str, default="data/minute_mysteries_qa/test.jsonl")
...
def check_answer(logs, label):
    ans = []
    for log in reversed(logs):
        response = log["content"]
        ans = re.findall(r"[tT]he correct option is.*?(\d)", response, re.DOTALL)
        if len(ans) != 0:
            break
...
def cli_main():
    ...
    for i, example in enumerate(dataloader):
        ...
        res.append(check_answer(logs, example["answer"]))
        f.write(
            json.dumps(
                {
                    "input": example["input"],
